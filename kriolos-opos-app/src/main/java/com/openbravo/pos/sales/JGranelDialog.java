...
/**
 * Diálogo para entrada de peso en productos de granel
 * Estilo Eleventa: cantidad a la izquierda, valor a la derecha, números grandes
 */
public class JGranelDialog extends JDialog {
    ...
    private JTextField txtCantidad; // Cantidad a la izquierda
    private JTextField txtValor; // Valor/Importe a la derecha
    ...
    private void initComponents() {
        ...
        // NO establecer botón por defecto para evitar que Enter active el botón
        // automáticamente
     ...
     * Se llama después de inicializar y puede ser llamado nuevamente si se
     * sobrescriben
     */
    private void aplicarFuentesGrandes() {
        if (txtCantidad != null) {
            Font fuenteCantidad = new Font("Segoe UI", Font.BOLD, 120);
            txtCantidad.setFont(fuenteCantidad);
            System.out.println("DEBUG: Aplicando fuente cantidad: " + fuenteCantidad.getSize() + " puntos");
        }
        if (txtValor != null) {
            Font fuenteValor = new Font("Segoe UI", Font.BOLD, 90);
    ...
    private CampoConPanel crearCampoSimple(String etiqueta, String valorInicial, boolean esCantidad) {
        ...
        int tamanoFuente = esCantidad ? 120 : 90; // Números más pequeños para casillas panorámicas
        ...
        campo.setBorder(BorderFactory.createCompoundBorder(
                ...
                BorderFactory.createEmptyBorder(5, 25, 5, 25) // Padding vertical reducido para evitar que se corten los números
        ));
        ...
        // Casillas panorámicas: más altas y anchas para mostrar los números completos
        int alturaCampo = 210; // Altura incrementada
    ...
    private void setupEvents() {
        ...
        KeyListener keyListener = new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
                    ...
                    // Cambiar a campo cantidad (izquierda)
                    txtCantidad.requestFocus();
                    txtCantidad.selectAll();
                    campoActivo = txtCantidad;
                    e.consume();
                } else if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
                    // Cambiar a campo valor (derecha)
        ...
        // ActionListener para Enter en los campos de texto (método estándar para
        // JTextField)
    ...
    private void incrementarCampoActivo() {
        if (campoActivo == txtCantidad) {
            ...
            try {
                ...
                // Si el valor está muy cerca de un múltiplo de 250, asumir que ya está en ese
                // múltiplo
    ...
    /**
     * Calcula el precio desde la cantidad, siempre actualizando el precio sin
     * importar el foco
     ...
     * También maneja entradas que empiezan con punto (ej: .750 -> 0.750)
     */
    private String normalizarFormatoNumero(String texto) {
        ...
        String resultado = texto.replace(",", ".").trim();

        // Si empieza con punto, agregar 0 antes (ej: .750 -> 0.750)
    ...
    private void aceptar() {
        ...
        synchronized (lockAceptar) {
            ...
            try {
                ...
                // Para diálogos modales, es crítico cerrar correctamente para que
                // setVisible(true) retorne
                ...
                // Forzar actualización de la UI para asegurar que el diálogo desaparezca
                // visualmente
                // Esto se ejecuta en el hilo de eventos para asegurar que se procese
                // correctamente
                SwingUtilities.invokeLater(() -> {
     ...
     * Similar al patrón usado en otros diálogos del código (JNumberDialog,
     * JEditorTextDialog)
     */
    ...
    private static class DecimalDocumentFilter extends DocumentFilter {
        ...
        private boolean isValidDecimal(String text) {
            ...
            // Permitir texto que sea solo un punto (el usuario está empezando a escribir
            // .750)
            if (text.equals("."))
                return true;

            // Si empieza con punto, agregar 0 antes para validar (ej: .750 -> 0.750)
            String textToValidate = text.startsWith(".") ? "0" + text : text;

            // Verificar que sea un número válido
            try {
                Double.parseDouble(textToValidate);
                return true;
