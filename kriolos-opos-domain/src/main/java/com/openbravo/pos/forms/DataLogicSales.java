...
public class DataLogicSales extends BeanFactoryDataSingle {
                ...
                return new StaticSentence(s,
                                new QBFBuilder(
                                                "SELECT "
                                                                ...
                                                                + "SUPPLIER, "
                                                                + "UOM, "
                                                                + "MEMODATE, " + "ACCUMULATES_POINTS "
                                                                + "FROM products "
                                                                + "WHERE ISCOM = " + s.DB.FALSE()
        ...
        public final List<CustomerTransaction> getCustomersTransactionList(String cId) throws BasicException {
                ...
                return new PreparedSentence<>(s,
                                "SELECT t.TICKETID, "
                                                ...
                                                + "CASE WHEN puntos_config.MONTO_POR_PUNTO > 0 AND cliente_puntos.ID IS NOT NULL THEN "
                                                + "    FLOOR(t.TOTAL_WITH_TAX / puntos_config.MONTO_POR_PUNTO) * puntos_config.PUNTOS_OTORGADOS "
                                                + "ELSE 0 END AS PUNTOS "
                                                + "FROM ("
                                                + "    SELECT tickets.ID AS TICKETID, "
                                                + "    COUNT(DISTINCT ticketlines.PRODUCT) AS PRODUCT_COUNT, "
                                                + "    MAX(products.NAME) AS PRODUCT_NAME, "
                                                + "    SUM(ticketlines.UNITS) AS TOTAL_UNITS, "
                                                + "    SUM(ticketlines.UNITS * ticketlines.PRICE) AS TOTAL_AMOUNT, "
                                                + "    SUM(ticketlines.UNITS * ticketlines.PRICE * (1.0 + taxes.RATE)) AS TOTAL_WITH_TAX, "
                                                + "    receipts.DATENEW, "
                                                + "    tickets.CUSTOMER AS CUSTOMER_ID "
                                                + "    FROM ticketlines "
                                                + "    INNER JOIN taxes ON (ticketlines.TAXID = taxes.ID) "
                                                + "    INNER JOIN tickets ON (tickets.ID = ticketlines.TICKET) "
                                                + "    INNER JOIN receipts ON (tickets.ID = receipts.ID) "
                                                + "    LEFT OUTER JOIN products ON (ticketlines.PRODUCT = products.ID) "
                                                + "    WHERE tickets.CUSTOMER = ? "
                                                + "    GROUP BY tickets.ID, receipts.DATENEW, tickets.CUSTOMER"
                                                + ") t "
                                                + "LEFT OUTER JOIN PUNTOS_CONFIGURACION puntos_config ON (puntos_config.SISTEMA_ACTIVO = TRUE) "
                                                + "LEFT OUTER JOIN CLIENTE_PUNTOS cliente_puntos ON (cliente_puntos.CLIENTE_ID = t.CUSTOMER_ID) "
                                                + "ORDER BY t.DATENEW DESC",
                                SerializerWriteString.INSTANCE,
                ...
                Transaction t = new Transaction(s) {
                        ...
                        public Object transact() throws BasicException {
                                ...
                                try {
                                        ...
                                        if (personRow != null) {
                                                ...
                                                if (currentUserName == null)
                                                        currentUserName = (String) personRow[1];
                                        }
                                } catch (Exception ignore) {
                                        // si falla, mantenemos null y usamos fallback abajo
                                }

                                // PERSON (receipts) ahora es el CARD del usuario actual; si no disponible, usar
                                // id previo
                                String receiptsPerson = currentUserCard != null ? currentUserCard
                                ...
                                params = new Object[] {
                                                ...
                                                ticket.getTicketType(),
                                                ticket.getTicketId(),
                                                ticket.getUser().getId(), // mantener PERSON en tickets como ID para no
                                                                          // romper joins existentes
                                        ...
                                        new PreparedSentence(s,
                                                        ...
                                                        .exec(new DataParams() {
                                                                ...
                                                                public void writeValues() throws BasicException {
                                                                        // El STATUS del reembolso guarda el TICKETID
                                                                        // del ticket original
                                                                        setInt(1, ticket.getTicketStatus());
                                                                }
                                                        });
                                }
                                ...
                                // Track cumulative delta per (product:attribute) so multiple lines in a ticket
                                ...
                                for (TicketLineInfo l : ticket.getLines()) {
                                        ...
                                        if (l.getProductID() != null && l.isProductService() != true) {
                                                String key = l.getProductID() + ":"
                                                                + (l.getProductAttSetInstId() == null ? ""
                                                                                : l.getProductAttSetInstId());
                                                double deltaUnits = -l.getMultiply(); // negative for sale
                                                double prevDelta = cumulativeDelta.containsKey(key)
                                                                ? cumulativeDelta.get(key)
                                                                : 0.0;
                                                ...
                                                if (currentStockCache.containsKey(key)) {
                                                        ...
                                                        currentUnits = findProductStock(location, l.getProductID(),
                                                                        l.getProductAttSetInstId());
                                                ...
                                                if (newDelta < 0.0 && currentUnits + newDelta < 0.0) {
                                                        ...
                                                        String productName = prod != null ? prod.getName()
                                                                        : l.getProductID();
                                                        throw new BasicException(
                                                                        "Insufficient stock for product " + productName
                                                                                        + " (id=" + l.getProductID()
                                                                                        + "). Available: "
                                                                                        + currentUnits + " Requested: "
                                                                                        + (-newDelta));
                                                }
                                                ...
                                                if (l.getProductID() == null) {
                                                        LOGGER.info(
                                                                        "Skipping stock update: Ticket line productID is null. Line: "
                                                                                        + l.getTicketLine());
                                                } else if (l.isProductService()) {
                                                        LOGGER.info("Skipping stock update: product is marked as SERVICE. Product ID: "
                                                                        + l.getProductID() + ", Name: "
                                                                        + l.getProductName());
                                                }
                                        }
         ...
         * @throws BasicException
         */
        public final void cancelarTicketDirecto(final TicketInfo ticket, final String location) throws BasicException {

                Transaction t2 = new Transaction(s) {
                        @Override
                        public Object transact() throws BasicException {

                                // 1. Marcar el ticket original como cancelado (STATUS = 2)
                                new PreparedSentence(s,
                                                "UPDATE tickets SET STATUS = 2 WHERE ID = ?",
                                                SerializerWriteParams.INSTANCE)
                                                .exec(new DataParams() {
                                                        @Override
                                                        public void writeValues() throws BasicException {
                                ...
                                for (int i = 0; i < ticket.getLinesCount(); i++) {
                                        ...
                                        if (line.getProductID() != null && !line.isProductService()) {
                                                getStockDiaryInsert().exec(new Object[] {
                                                                UUID.randomUUID().toString(),
                                                                d,
                                                                MovementReason.IN_REFUND.getKey(),
                                                                ...
                                                                line.getMultiply(), // positivo = devuelve al stock
                                                                line.getPrice(),
                                                                ticket.getUser() != null ? ticket.getUser().getName()
                                                                                : "System"
                                                });
                                        }
                                }
        ...
        public final void deleteTicket(final TicketInfo ticket, final String location) throws BasicException {
                ...
                t = new Transaction(s) {
                        ...
                        public Object transact() throws BasicException {
                                        ...
                                        if (bundle.size() > 0) {
                                                for (ProductsBundleInfo bundleComponent : bundle) {
                                                        ...
                                                        if (bundleProduct != null && bundleProduct.isService()) {
                                                                LOGGER.info("Skipping bundle component stock update: product is a SERVICE. Product ID: "
                                                                                + bundleComponent.getProductBundleId());
                                                                continue;
                                                        }
                                                        getStockDiaryInsert().exec(new Object[] {
                                ...
                                for (int i = 0; i < ticket.getLinesCount(); i++) {
                                        if (ticket.getLine(i).getProductID() != null
                                                        && ticket.getLine(i).getProductName() != null) {
        ...
        public final SentenceExec getStockDiaryInsert() {
                return new SentenceExecTransaction(s) {
                        ...
                        public int execInTransaction(Object[] params) throws BasicException {
                                ...
                                LOGGER.info(
                                                "<<< getStockDiaryInsert.execInTransaction - adjustStock completado, insertando en stockdiary");
                ...
                if (bundle.size() > 0) {
                        ...
                        for (ProductsBundleInfo component : bundle) {
                                ...
                                if (compProd != null && compProd.isService()) {
                                        LOGGER.info("Skipping stock adjustment for bundle component SERVICE product: "
                                                        + component.getProductBundleId() + " - "
                                                        + (compProd != null ? compProd.getName() : "<unknown>"));
                                        continue;
                                }
                                Object[] adjustParams = new Object[4];
                        ...
                        double stockAntes = findProductStock(locationId, productId,
                                        attSetInstId != null ? attSetInstId.toString() : null);
                        LOGGER.info("    Stock ANTES del ajuste: " + stockAntes);

                        // Para productos sin atributos, consolidar TODOS los registros (con y sin
                        // atributos) en uno solo
                        ...
                        if (attSetInstId == null) {
                                ...
                                // atributos)
                                Double totalStockAll = (Double) new PreparedSentence(s,
                                                "SELECT SUM(UNITS) FROM stockcurrent "
                                                                + "WHERE LOCATION = ? AND PRODUCT = ?",
                                                new SerializerWriteBasic(Datas.STRING, Datas.STRING),
                                ...
                                try {
                                        ...
                                        int insertResult = new PreparedSentence(s,
                                ...
                                if (updateresult == 0) {
                                        LOGGER.info("    No existe registro con estos atributos - Ejecutando INSERT");
                                        Object[] insertParams = new Object[4];
                                        insertParams[0] = locationId;
                                        insertParams[1] = productId;
                                        insertParams[2] = attSetInstId;
                                        insertParams[3] = units;
        ...
        public final double findProductStock(String warehouse, String id, String attsetinstid) throws BasicException {
                ...
                LOGGER.info("    [findProductStock] Location: " + warehouse + ", Product: " + id + ", AttSetInstId: "
                                + attsetinstid + " -> Stock: " + result);
        ...
        public final SentenceList<UomInfo> getUomList() {
                return new StaticSentence(s, "SELECT ID, NAME  FROM uom ORDER BY NAME", null,
